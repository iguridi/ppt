...
class Maker:
            ...
            if name != 'salmo':
                if name == 'segunda_lectura':
                    try:
                        ...
                        continue
                self.reading = Reading(name, self.addrs[name],
                                       self.readings[name])
            else:
                self.reading = Psalm(name, self.addrs[name],
                                     self.readings[name])
            self.separate_text()
    ...
    def make_readings_slides(self):
        ...
        for i, slide_text in enumerate(self.reading.slides):
            ...
            if self.reading.title == 'salmo':
                ...
                resp = txt_fm.paragraphs[
                    0]  # the default paragraph has white space. Dont have to add a new one, you have to use it
    ...
    def add_readings_ends(self, slide):
        endings = {
            'primera_lectura': ('Palabra de Dios', 'Te alabamos Señor'),
