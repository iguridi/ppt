...
def pulirDireccion(texto_general):
    ...
    for palabra in texto_general.split(" "):  # Buscar palabra por palabra
        ...
        for (
            libro
        ) in libros:  # Buscar un libro(en forma de diccionario) en la lista de libros
            if libro["Name"] == palabra:
                lbr = libro["Name"]
                lbrAbrev = libro[
                    "Abreviattion"
                ]  # Replace the book name for its shortening
                break
        if lbrAbrev:
            break
...
def find_first_reading(text):
    rough_first_reading = text[
        text.find(first_reading_name) : text.find(salmo_responsorial_name)
    ]
...
def find_second_reading(text):
    try:
        ...
        return
    rough_second_reading = text[
        text.find(second_reading_name) + 15 : text.find("SECUENCIA")
    ]
    return rough_second_reading


def find_gospel(gospel_text):
    rough_gospel = gospel_text[
        gospel_text.find("Evangelio de nuestro Señor Jesucristo")
        + 48 : gospel_text.find(credo_name)
        - 1
    ]
    return rough_gospel
